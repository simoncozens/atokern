...
import keras
import os
from settings import output_path, generate, mirroring, covnet, augmentation, batch_size, dropout_rate, init_lr, lr_decay, input_names, regress, threeway, trust_zeros, mu, binfunction, kern_bins, training_files, validation_files, all_pairs, width, depth, is_bin
...
for n in input_names:
  ...
  if covnet:
    conv = Activation("relu")(Conv1D(16,8)(input_))
    #maxp = MaxPooling1D(pool_size=2)(conv)
    flat = Flatten()(conv)
    net = flat
    net = relu(net,layers=1,nodes=512)
  else:
    net = input_
...
if is_bin >= 0:
  kernvalue = Dense(1, activation="sigmoid")(x)
elif regress:
